...
def run():
    ...
    print("INFO | Purging partial semantic edges...")
    cur.execute("DELETE FROM edges WHERE provenance = 'OEWN_2025';")
    conn.commit()

    print("INFO | Indexing staging synset members for join acceleration...")
    cur.execute("""
        CREATE INDEX IF NOT EXISTS staging.idx_stg_sem_lookup 
        ON staging_semantic_relations(synset_id, relation_type, lemma, pos);
    """)

    print("INFO | Streaming all inter-synset relations via synset members...")
    cur.execute("""
    SELECT DISTINCT 
    ...
    print(f"PASS | Semantic Edges Ingested: {total_edges:,} | Duration: {time.time() - t0:.2f}s")
    print(f"EDGE DISTRIBUTION: {dist}")
